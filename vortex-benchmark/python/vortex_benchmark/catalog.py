...
WORKLOAD_DEFINITIONS = (
    ...
    WorkloadDefinition(
        name="hot_counter",
        description="All clients issue INCRBY against one hot integer key.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=0,
        write_ratio=100,
        multi_key=False,
        transactional=False,
        hot_key=True,
        default_command_groups=("strings",),
        default_commands=("INCRBY",),
    ),
    WorkloadDefinition(
        name="hot_counter_with_get",
        description="Hot integer counter with 90 percent INCRBY and 10 percent GET.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=10,
        write_ratio=90,
        ...
        hot_key=True,
        default_command_groups=("strings",),
        default_commands=("INCRBY", "GET"),
    ),
    WorkloadDefinition(
        name="hot_counter_with_set_del",
        description="Hot counter barrier stream with 80 percent INCRBY, 10 percent SET, and 10 percent DEL.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=0,
        write_ratio=100,
        ...
        hot_key=True,
        default_command_groups=("strings", "keys"),
        default_commands=("INCRBY", "SET", "DEL"),
    ),
    WorkloadDefinition(
        name="hot_counter_ttl",
        description="Hot counter with a live TTL, using mostly INCRBY plus TTL probes.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=10,
        write_ratio=90,
        ...
        hot_key=True,
        default_command_groups=("strings", "keys"),
        default_commands=("SETEX", "INCRBY", "TTL"),
    ),
    WorkloadDefinition(
        name="hot_counter_transaction",
        description="Hot counter increments wrapped in MULTI/EXEC transaction boundaries.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=0,
        write_ratio=100,
        ...
        default_commands=("MULTI", "INCRBY", "EXEC"),
    ),
    WorkloadDefinition(
        name="hot_counter_watch_transaction",
        description="Hot counter increments under WATCH plus MULTI/EXEC optimistic transaction semantics.",
        distribution="single-hot-key",
        key_pattern="single-hot-counter",
        read_ratio=0,
        write_ratio=100,
        ...
        hot_key=True,
        default_command_groups=("strings", "transactions"),
        default_commands=("WATCH", "MULTI", "INCRBY", "EXEC"),
    ),
    WorkloadDefinition(
        name="hot_counter_multikey",
        description="Hot counter mixed with MGET/MSET operations that include the counter key.",
        distribution="single-hot-key",
        key_pattern="multi-key-hot-counter",
        read_ratio=10,
        write_ratio=90,
        multi_key=True,
        transactional=False,
        hot_key=True,
        default_command_groups=("strings",),
        default_commands=("INCRBY", "MGET", "MSET"),
    ),
    WorkloadDefinition(
    ...
    WorkloadDefinition(
        name="mset_only",
        description="Pure MSET operations for multi-key write latency and tail analysis.",
        distribution="uniform",
        key_pattern="multi-key",
        read_ratio=0,
        write_ratio=100,
        ...
        hot_key=False,
        default_command_groups=("strings",),
        default_commands=("MSET",),
    ),
    WorkloadDefinition(
        name="mget_only",
        description="Pure MGET operations for multi-key read latency and tail analysis.",
        distribution="uniform",
        key_pattern="multi-key",
        read_ratio=100,
        write_ratio=0,
        ...
        hot_key=False,
        default_command_groups=("strings",),
        default_commands=("MGET",),
    ),
    WorkloadDefinition(
        name="msetnx_only",
        description="Pure MSETNX operations for multi-key conditional-write latency and tail analysis.",
        distribution="uniform",
        key_pattern="multi-key",
        read_ratio=0,
        write_ratio=100,
    ...
    WorkloadDefinition(
        name="pressure-slow_reader",
        description="One slow reader plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
        ...
        transactional=False,
        hot_key=False,
        default_command_groups=("server", "strings"),
        default_commands=("PING", "GET"),
    ),
    WorkloadDefinition(
        name="pressure-deep_pipeline",
        description="One deep-pipeline client plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
    ...
    WorkloadDefinition(
        name="pressure-large_bulk",
        description="One large-bulk writer plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
        ...
        hot_key=False,
        default_command_groups=("strings",),
        default_commands=("GET", "SET"),
    ),
    WorkloadDefinition(
        name="pressure-aof_backlog",
        description="One durable-write pressure client plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
    ...
    WorkloadDefinition(
        name="pressure-ttl_expiry",
        description="One TTL churn client plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
        ...
        hot_key=False,
        default_command_groups=("strings", "keys"),
        default_commands=("GET", "SET", "PEXPIRE", "PTTL"),
    ),
    WorkloadDefinition(
        name="pressure-eviction_pressure",
        description="One eviction-pressure writer plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
    ...
    WorkloadDefinition(
        name="pressure-connection_storm",
        description="Connection churn plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
    ...
    WorkloadDefinition(
        name="pressure-close_storm",
        description="Accept-and-close churn plus latency-sensitive pipeline-1 GET clients.",
        distribution="uniform",
        key_pattern="single-key",
        read_ratio=100,
        write_ratio=0,
